                ...
                selections: [ // 가전 별 선택 가능한 영역
                    {
                        ...
                        Surface: [
                            ...
                            // m 그린 이미지 경로 
                            // "images/objet/simulator/appliances/rf/rf_door01_02st_green.png"
    ...
    Simulator.prototype = {
        ...
        initObjects: function () {
            ...
            if (stageSetting.objectArea !== null) {
                ...
                for (var i = 0; i < configData.object.length; i++) {
                    outputHtml += '<div class="objet 민주입니다!!!!" data-objet-id="' + configData.object[i].id + '">' +
                ...
                // 초록, 실버, 실버 color 삽입 default
                document.getElementById('refrigerator_LT').src = 'images/objet/simulator/appliances/rf/rf_door01_02st_green.png';
                document.getElementById('refrigerator_LB').src = 'images/objet/simulator/appliances/rf/rf_door03_02st_silver.png';
        ...
        setObject: function (ID, targetID, colorCode, boolIsSet, colorSetName) {  // @2020-10-27 pck 인기조합 여부 변수 추가
            console.log("컬러 선택시 이벤트?")
            console.log('ID : ' + ID)
            console.log('targetID : ' + targetID)
            console.log('colorCode : ' + colorCode)
            console.log('boolIsSet : ' + boolIsSet)
            console.log('colorSetName : ' + colorSetName)
            var _self = this;
            if (ID == null)
                return false;
            if (colorCode == null)
            ...
            if (existObjects[0].selectedObject_desc.length > 0 && !isFavSet && !isPassCheckColorGroup) {
                ...
                if (!colorGroupCheck()) { //서로 다른 그룹 호출 시 안내 alert 호출
                    console.log("m - 제품색상이 없을시!");
                    // color_warning_popup 엘리먼트가 있을때? 를 그 엘리먼트를 변수에 넣는다 그렇지 않으면 false 임;
                    var colorWarningPopup = document.querySelector('.color_warning_popup') !== null ? document.querySelector('.color_warning_popup') : false;
                    //if(colorWarningPopup.classList.contains('actived')) colorWarningPopup = false;

                    if (colorWarningPopup) {
                        console.log(colorWarningPopup);
                        colorWarningPopup.style.display = 'block';
                        colorWarningPopup.classList.add('actived');
                        let selObj = $(".object-app.active").attr("data-object-selector"); // data-object-selector 의 값
        ...
        updateSideBar: function (ID, targetID) {
            ...
            */
            ...
            var outputHtml = '';
            var classes = '';
            // console.log(configData.object[0].selections[0].price

            // 냉장고 클릭시 팝업 생성
            for (var i = 0; i < arraySelectedSurface.length; i++) {
                console.log('c',ID,targetID)
                outputHtml += '<div class="select_objet_item 팝업생성부분!">' + '<p class="name">' + arraySelectedSurface[i].name + '<button type="button" class="btn-Tooltip" onClick="tooltipOpen(this);" value="' + arraySelectedSurface[i].name + '">Tooltip</button></p>' + '<ul>';
            ...
            if (stageSetting.objetSelectedListArea !== null) {
                ...
                */
                ...
                for (var i = 0; i < configData.object.length; ++i) {
                    ...
                    console.log(selectedObjectComplete)
